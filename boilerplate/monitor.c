 ...
 *
 * Fixed for kernel 5.4 compatibility:
 *   - Added <linux/sched/mm.h> for get_task_mm / mmput
 *   - All variable declarations moved to top of blocks (C89 style)
 *   - No mixed declarations and code anywhere
 */
...
#include <linux/sched/mm.h>      /* get_task_mm, mmput -- needed on 5.4 */
...
/* RSS helper -- returns bytes, or -1 if process gone */
...
static void log_soft_limit_event(const char *cid, pid_t pid,
...
static void kill_process(const char *cid, pid_t pid,
                          unsigned long limit, long rss)
{
    struct task_struct *task;
...
/* IOCTL handler -- all declarations at top (C89) */
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        list_add_tail(&e->node, &monitored_list);
        mutex_unlock(&monitored_lock);
        return 0;
    }
